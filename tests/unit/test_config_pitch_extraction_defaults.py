...
def test_training_config_convergence_fix_defaults():
    ...
    assert config.max_lr_multiplier == 1.5, (
        f"max_lr_multiplier should be 1.5; got {config.max_lr_multiplier}"
    )

    # Encoder gets a separate higher LR to compensate for its much smaller Adam 2nd moments
    assert config.encoder_lr_multiplier == 1.5, (
        f"encoder_lr_multiplier should be 1.5; got {config.encoder_lr_multiplier}"
    )
